...
interface WordDao {
    ...
    @Query(
        """
        SELECT * FROM words
        WHERE normalizedEnglishWord LIKE '%' || LOWER(TRIM(:query)) || '%'
            OR LOWER(indonesianMeaning) LIKE '%' || LOWER(TRIM(:query)) || '%'
            OR LOWER(category) LIKE '%' || LOWER(TRIM(:query)) || '%'
        ORDER BY category ASC, englishWord ASC
        """
    )
    ...
    @Query(
        ...
        SELECT words.* FROM words
        INNER JOIN word_progress ON words.id = word_progress.wordId
        WHERE word_progress.wrongCount > 0
        ORDER BY word_progress.wrongCount DESC, word_progress.correctCount ASC, words.englishWord ASC
        LIMIT :limit
        """
    )
    ...
    @Query("SELECT * FROM words WHERE normalizedEnglishWord = :normalizedEnglishWord LIMIT 1")
    suspend fun getWordByNormalizedEnglish(normalizedEnglishWord: String): WordEntity?

    @Query(
        """
        SELECT COUNT(*) FROM words
        WHERE normalizedEnglishWord = :normalizedEnglishWord
            AND id != :excludedWordId
        """
    )
    suspend fun countDuplicateNormalizedEnglish(
        normalizedEnglishWord: String,
        excludedWordId: Long = 0
    ): Int
    ...
    @Query(
        """
        SELECT category, COUNT(*) AS wordCount
        FROM words
        GROUP BY category
        ORDER BY wordCount DESC, category ASC
        """
    )
